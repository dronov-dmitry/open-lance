    ...
    window.saveProfile = async () => {
        ...
        try {
            const updates = {
                ...
                avatar_url: document.getElementById('edit-profile-avatar').value.trim()
            };
        ...
        if (isEdit) {
            container.innerHTML = `
                <div class="card" style="max-width: 600px; margin: 0 auto;">
                    ...
                    <div class="form-group">
                        <label for="edit-profile-avatar">URL Аватара (ссылка на картинку)</label>
                        <input type="url" id="edit-profile-avatar" value="${p.avatar_url || ''}" placeholder="https://example.com/avatar.png">
                    </div>
                ...
                <div class="card" style="max-width: 600px; margin: 0 auto; text-align: center;">
                    ...
                    <div style="text-align: left; margin-bottom: 30px;">
                        <div style="display: flex; justify-content: space-between; align-items: baseline; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-bottom: 15px;">
                            ...
                            ${!isOwnProfile && window.auth.isLoggedIn() ? `
                                <button class="btn btn-outline" style="padding: 4px 10px; font-size: 0.8rem;" onclick="window.toggleReviewForm()">Оставить отзыв</button>
                            ` : ''}
                        </div>
                        
                        ${!isOwnProfile && window.auth.isLoggedIn() ? `
                        <div id="inline-review-form-profile" style="display: none; margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background: #fafafa;">
